...
int main(){
  ...
  for (int x = 0; x < WIDTH; x++)
   for (int y = 0; y < HEIGHT; y++) 	maze[y][x] = ' ';
  // Borders with X
  for (int x = 0; x < WIDTH; x++)
  { 	maze[0][x] = 'X'; 	maze[HEIGHT-1][x] = 'X';
  }
  for (int y = 0; y < HEIGHT; y++)
  { 	maze[y][0] = 'X'; 	maze[y][WIDTH-1] = 'X';
  } 
  ...
  // The array is of size [h-2]*[w-2] since we'll never exceed that size.
  ...
  int numPoints = 0;  
  bool found = solver.search(maze, visited, x, y, solutionX, solutionY, numPoints);
  ...
  { 	
    ...
    { 		
      solver.printMaze(maze, solutionX[i], solutionY[i]); 		
      cout << endl;
  	}
  }
}
